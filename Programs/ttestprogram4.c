...
int main() {
    ...
    // Assuming a two-tailed test with alpha = 0.05 and degrees of freedom = n-1
    // You can compare the t-value with critical t-value from t-distribution table to determine significance
    // For simplicity, we are not doing that here
    printf("Null Hypothesis: The mean of the sample does %s differ significantly from the assumed mean.\n", fabs(t_value) > 2.0 ? "" : "not");
