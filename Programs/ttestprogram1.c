...
double get_critical_t_value(int df, double alpha, int one_tailed) {
    // Hardcoded critical values for df = 8 and common alpha values
    if (df == 8) {
        if (!one_tailed) { // two-tailed
...
int main() {
    ...
    // Read whether the test is one-tailed (1 for yes, 0 for no)
    printf("Is this a one-tailed test? (1 for yes, 0 for no): ");
    scanf("%d", &one_tailed);
    ...
    double critical_t_value = get_critical_t_value(df, alpha, one_tailed);
